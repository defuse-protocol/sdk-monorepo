...
describe('validateZcashAddress', () => {
    ...
    it('accepts a valid transparent address', () => {
        expect(validateAddress(validTransparentAddress, BlockchainEnum.zec)).toBe(true);
    });
    ...
    it('rejects a transparent address with invalid characters', () => {
        expect(validateAddress(invalidTransparentAddress, BlockchainEnum.zec)).toBe(false);
    });
