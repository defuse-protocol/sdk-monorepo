...
export class MpcContract {
    ...
    private buildSignMpcPromiseSingle(domainId: DomainId, payload: Uint8Array, path: string): PromiseSingle {
        ...
        switch (domainId) {
            case DomainId.Secp256k1: {
                if (payload.length !== 32) {
                    throw new Error("Invalid payload length for Secp256k1. Expected length 32")
                }
            ...
            case DomainId.Ed25519: {
                if (payload.length < 32 || payload.length > 1232) {
                    throw new Error("Invalid payload length for Ed25519. Expected length must be between 32 and 1232")
                }
        ...
        return {
            receiver_id: this.contractId, actions: [{
    ...
    buildSignMpcRequest(domainId: DomainId, payload: Uint8Array, path: string = ""): Request {
        return {
            ops: [], out: {
                after: [],
