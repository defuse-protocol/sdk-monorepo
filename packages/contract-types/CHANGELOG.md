...
- be5c0fb: fix: tip191 (Tron) and sep53 (Stellar) validation in MultiPayloadValidator
  ...
  parseJson keyword was mutating payload data during oneOf evaluation, corrupting sibling branches. Deferred mutations
  until after AJV selects the matching branch.
...
- fecef80: Add Standard Schema compliant validators with lazy compilation and type inference helpers
  ...
  Works with any Standard Schema compatible library. See test files for copyable wrapper code:
    ...
    - **class-validator (NestJS)** - see `class-validator.wallet-contract-variant.ts` for `ValidateWithSchema` decorator
    - **Zod** - see `zod.wallet-contract-variant.ts` for `toZodSchema()` wrapper
    - **Valibot** - see `valibot.wallet-contract-variant.ts` for `toValibotSchema()` wrapper
    - **ArkType** - see `arktype.wallet-contract-variant.ts` for `toArkTypeSchema()` wrapper
